...
// 2. Add New Customer (with multi-part image upload handling)
router.post('/add', upload.single('image'), async (req, res) => {
  try {
    const bodyData = { ...req.body };
    if (req.file) {
      // Dynamic network string handler logic swaps development domain with your production domain cleanly
      const hostUrl = req.get('host').includes('localhost') ? 'http://localhost:5000' : 'https://onrender.com';
      bodyData.imageUrl = `${hostUrl}/uploads/${req.file.filename}`;
    }
    const newCustomer = new Customer(bodyData);
    await newCustomer.save();
    res.status(201).json({ success: true, data: newCustomer });
